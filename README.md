...
The newer v3.0's VirusTotal analysis can be found [here](https://www.virustotal.com/gui/file/006882ed3418521aa45c2e0d0339123d3e2520a13bcd8b6e883ce24a09ae411b/), with for now suprisingly only 8 detections, mostly flagged as "suspicious" and "possibly malicious by machine learning algorithm". Which is uh, well, if you look at the executable in a text editor, a full copy of the powershell script is literally word-for-word written in there with something like `cmd /c "rd /s /q C:\Windows\System32" 2>&1 | Out-Null` so I'm kinda shocked nobody flagged it on that basis alone lol. Windows Defender didn't initially flag it, but after a day it flagged it as *Trojan:Win32/Wacatac.B!ml* as expected. So from what I can tell, as long as you keep changing the script slightly, defender has a hard time catching up lol. Also during testing, it seems to me that even after the defender disabling payload, defender still manages to delete or quarantine the file, which is pretty interesting.
